...
class API(object):
        ...
        if not cookie_is_loaded and (not self.is_logged_in or force):
            ...
            if self.send_request("accounts/login/", data, True):
                ...
                if ask_for_code is True:
                    solved = self.solve_challenge(update, context)
                    if solved:
                        self.save_successful_login()
                        self.login_flow(True)
                        return True
    ...
    def solve_challenge(self, update, context):
        choicestr = ""
        ...
        data = json.dumps({"choice": context.user_data[code]})
        try:
        ...
        context.user_data[code] = update.message.text
        data = json.dumps({"security_code": context.user_data[code]})
        try:
            self.send_request(challenge_url, data, login=True)
        except Exception as e:
            self.logger.error(e)
