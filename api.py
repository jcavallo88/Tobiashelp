...
class API(object):
        ...
        if not cookie_is_loaded and (not self.is_logged_in or force):
            ...
            if self.send_request("accounts/login/", data, True):
                ...
                if ask_for_code is True:
                    solved = self.solve_challenge(update, context, check)
                    if solved:
                        self.save_successful_login()
                        self.login_flow(True)
                        return True
    ...
    def solve_challenge(self, update, context, check):
        choicestr = ""
        challenge_url = self.last_json["challenge"]["api_path"][1:]
        if challenge_url == None:
            return True
        try:
        ...
        for choice in choices:
            choicestr += (choice + "\n")
        context.bot.send_message(chat_id=update.effective_chat.id, text="Please choose:" + "\n" + choicestr)
        code1 = update.message.text
        context.user_data["newcode"] = code1
        while context.user_data["newcode"] == check:
            print(check)
            code1 = update.message.text
            context.user_data["newcode"] = code1
            print(update.message.text)
        print(context.user_data["newcode"])
        data = json.dumps({"choice": context.user_data["newcode"]})
        try:
            self.send_request(challenge_url, data, login=True)
        except Exception as e:
            self.logger.error(e)
            return False       
        context.bot.send_message(chat_id=update.effective_chat.id, text="A code has been send to the method chosen please reply with it here")
        while context.user_data["newcode"] == code1:
            code2 = update.message.text
            context.user_data["newcode"] = code2
        data = json.dumps({"security_code": context.user_data["newcode"]})
        try:
            self.send_request(challenge_url, data, login=True)
        except Exception as e:
            self.logger.error(e)
